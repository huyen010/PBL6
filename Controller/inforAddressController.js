...
exports.updateInforAddress = async (req, res) => {
  try {
    ...
    inforAddress = await InforAddress.findByIdAndUpdate(id, {
      name: name,
      phone: phone,
      address: address,
      role: role,
...
exports.getInforAddresss = async (req, res) => {
  try {
    ...
    var inforAddress = await InforAddress.find({ id_account: id_account })
      .populate("address.id_province", ["name", "region"])
      .populate("address.id_district", ["name"])
      .populate("address.id_commune", ["name"]);
    if (!inforAddress) {
      res.status(404).json({
        message: "Not availble",
        status: false,
