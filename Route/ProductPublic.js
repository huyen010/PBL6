...
router.post("/search/:slug/:page/:sort", async function (req, res) {
  ...
  if (req.params.sort == 3) {
    sortType = { price: -1 };
  }
  if (req.params.sort == 4) {
    sortType = { price: 1 };
  }
  if (req.params.sort == 5) {
    sortType = { name: -1 };
  }
  if (req.params.sort == 6) {
    sortType = { name: 1 };
  }
  let count = 0;
  try {
...
router.get("/detail/:slug", async function (req, res) {
  ...
  try {
    ...
    const rs = await StockProduct.find({ id_product: product._id }).select([
      "size",
      "color",
      "number",
    ]);
    ...
    const comment = await Comment.find({ id_product: product._id }).populate(
      "id_account",
      ["name"]
    );
    return res
      .status(200)
      .send({ product: product, rate: rate, comment: comment, number: rs });
  } catch (err) {
...
router.get("/wish-list/:page", async function (req, res) {
  ...
  try {
    const products = await Rate.find({ rate: { $gte: 4 } })
      .sort({ rate: 1 })
      .limit(16)
      .skip((page - 1) * 16)
      .populate({
      ...
      .populate("id_product.size", ["name"])
      .populate("id_product.color", ["name"]);
