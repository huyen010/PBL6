...
router.delete("/delete/:id", async function (req, res) {
  try {
    ...
    if (supply.listProduct.length > 0) {
      ...
      let representative = await Representative.findByIdAndDelete(
        supply.id_representative
      );
...
router.get("/all/:page", async function (req, res) {
  try {
    ...
    const supplies = await Supply.find({})
      ...
      .skip((page - 1) * 10)
      .sort({ _id: -1 })
      .populate("id_representative", ["name", "phone", "possition", "image"])
      .populate("listProduct", "name")
      .populate({
        path: "id_commune",
        select: "name",
        populate: {
...
router.put("/update/:id", async function (req, res) {
  try {
    ...
    let representative = await Representative.findByIdAndUpdate(
      supply.id_representative,
      {
        phone: req.body.representative.phone,
        name: req.body.representative.name,
        possition: req.body.representative.possition,
      }
    );
    // let supply = new Supply({name:req.body.name,phone:req.body.phone,street:req.body.street,id_commune:req.body.id_commune,id_representative:req.body.id_representative});
    res.status(200).send(supply);
