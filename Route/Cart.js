...
router.get("/", auth, async function (req, res) {
  try {
    let cart = await Cart.findOne({ id_account: req.user.id })
      .populate("product.id_product", ["name", "price", "urlImage", "discount"])
      .populate("product.color", ["name"])
      .populate("product.size", ["name"]);
    let price = 0;
    for (let i = 0; i < cart.product.length; i++) {
      price =
        price +
        cart.product[i].id_product.price *
          (1 - cart.product[i].id_product.discount / 100) *
          cart.product[i].number;
    }
...
router.post("/insert", auth, async function (req, res) {
  try {
    ...
    let pd = listPD.filter(
      (listPD) =>
        (listPD.id_product.toString() === req.body.id_product) &
        (listPD.size.toString() === req.body.size) &
        (listPD.color.toString() === req.body.color)
    );
    if (pd.length == 0) {
      cart.product.push({
...
router.put("/update", auth, async function (req, res) {
  try {
    let cartupdate = await Cart.updateOne(
      {
        id_account: req.user.id,
        product: { $elemMatch: { _id: req.body.id } },
      },
      {
        $set: {
          "product.$.number": req.body.number,
    ...
    const cartAfter = await Cart.findOne({ id_account: req.user.id })
      .populate("product.id_product", ["name", "price", "urlImage", "discount"])
      .populate("product.color", ["name"])
      .populate("product.size", ["name", "description"]);
    let price = 0;
    for (let i = 0; i < cartAfter.product.length; i++) {
      price =
        price +
        cartAfter.product[i].id_product.price *
          (1 - cartAfter.product[i].id_product.discount / 100) *
          cartAfter.product[i].number;
    }
